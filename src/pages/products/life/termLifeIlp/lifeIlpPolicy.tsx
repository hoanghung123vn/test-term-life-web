...
import { Box, Dialog, Grid, Table, Column, Input, DatePicker, Button, Space, Select, Card, Email, Icon, Number, Notice} from '@ebaoui/rainbowui-sugar-etmoui';
...
const LifePolicyInfo = () => {
    ...
    const [storeLifePolicy, setLifePolicy] = useState(location.state.lifeIlpPolicy);

    const [ifModify, setIfModify] = useState(false)
    const [fundDialogVisible, setFundDialogVisible] = useState(false);
    const [fundDisplay, setFundDisplay] = useState(false);
    ...
    const [productCode, setProductCode] = useState('RegularILP_PayWL_ByPrem')
    const [productId, setProductId] = useState()
    const [selectRowKeys, setSelectRowKeys] = useState<any>([]);
    const [renderFundList, setRenderFundList] = useState<any>([])
    ...
    const onSelectionChange = (keys, data) => {
    ...
    const getProductFund = async (productCode) => {
        AjaxUtil.show();
        const allInfoUrl = UrlUtil.getConfigUrl('UI_API_GATEWAY_PROXY', 'EXTERNALBFF', 'LIFE_PRODUCT_ALL_INFO');
        const allInfoResult = await AjaxUtil.call(allInfoUrl+`${productCode}`, null, { 'method': 'GET' });
        AjaxUtil.hide();
        setProductId(allInfoResult.product.productId)
        const productId = allInfoResult.product.productId

        AjaxUtil.show();
        const url = UrlUtil.getConfigUrl('UI_API_GATEWAY_PROXY', 'EXTERNALBFF', 'LIFE_PRODUCT_FUND');
        const result = await AjaxUtil.call(url+`?productId=${productId}`, null, { 'method': 'GET' });
        AjaxUtil.hide();
        setDataSource(result.body.data)
    }
    ...
    const saveMainFund = () => {
        ...
        for (let i = 0; i < dataSource.length; i++) {
            if (selectRowKeys.includes(dataSource[i].fundCode)) {
                tmp.push(dataSource[i])
            }
        }
        const total = tmp.reduce((previousValue, currentValue) => (parseFloat(previousValue.percent) + parseFloat(currentValue.percent)).toFixed(2));
        if (parseFloat(total) != 1.00) {
            Notice.error({
                title: 'Error',
                content: 'Each apportion should be no less than 1 and total should be 100%'
            });
        } else {
            setRenderFundList(tmp)
            closeDialog()
        }
    }

    const onChangeDataSource = (value, index) => {
    ...
    const composeQuotationRequest = () => {
        ...
        const investList = renderFundList.map(fund => {return {fundCode: fund.bizFundCode, assignRate: fund.percent}})
        newPolicyRequest.coverages[0].premInvestRates = investList
    ...
    const next = async () => {
        ...
        const newLifePolicy = { ...storeLifePolicy, policy: newPolicy };
        setLifePolicy(newLifePolicy);
        navigate('/lifeIlpPay', { state: { lifeIlpPolicy: newLifePolicy } });
    }
    ...
    return (
        <>
            ...
            <Card.Group>
                ...
                <Card noI18n outline="none" title="Main Benefit">
                    {
                        renderFundList.length > 0 &&
                        <Table
                            dataSource={renderFundList}
                            idField='fundCode'
                        >
                            <Column noI18n title="#" render={
                                (data, index) => <span>{index+1}</span>
                            } />
                            <Column noI18n title="Fund Code" dataIndex="bizFundCode" />
                            <Column noI18n title="Fund Name" dataIndex="fundName" />
                            <Column noI18n title="Percent" render={(data, index) =>
                                <Number type="percent" rate={100} value={data.percent}
                                    io='out'/>}
                            />
                        </Table>
                    }
                    <Button noI18n value={renderFundList.length > 0 ? 'Edit Fund' : 'Add Fund'} type='text' prevIcon='AddFill'
                        onClick={() => setFundDialogVisible(true)}/>
                    {/*<Icon name='AddFill' state='info'/>*/}

                </Card>

                {/*<Button noI18n value="Add Fund" className='btn' onClick={() => set} />*/}
            </Card.Group>
            <Dialog
                noI18n
                title={renderFundList.length > 0 ? 'Edit Fund' : 'Add Fund'}
                style={{width: '800px'}}
                ...
                footer={
                    <Space style={{justifyContent: 'flex-end'}}>
                        ...
                        <Button
                            ...
                            style={{width: 80}}
                            type="primary"
                            onClick={(e, pass) => pass && saveMainFund()}
                        />
                    </Space>
                }
            >
                ...
                >
                    ...
                    <Column noI18n title="Percent" render={(data, index) =>
                        <Number type="percent" rate={100} value={data.percent}
                            io={!selectRowKeys.includes(data.fundCode)?'out' : 'in'}
                            onChange={(value) => onChangeDataSource(value, index)}/>}
                    />
                </Table>
            </Dialog>
            <Box mode="footer">
                <Space direction="right" className='policyFooter' style={{ justifyContent: 'flex-end' }}>
                    ...
                    <Button value="Back" className='btn' onClick={() => navigate('/lifeIlpCoverage')} />
                </Space>
            </Box>
